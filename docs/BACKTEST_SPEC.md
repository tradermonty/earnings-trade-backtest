...
決算発表後のポジティブサプライズとギャップアップを狙ったスイングトレード戦略のバックテストシステムです。
...
| フィルター | 条件 | 設定パラメータ |
|-----------|------|----------------|
| 時価総額 | $5B以上 | `min_market_cap: 5e9` |
| 時価総額上限 | なし | `max_market_cap: 0` |
| 平均出来高 | 200K株以上 | `screener_volume_min: 200_000` |
| 株価 | $30以上 | `screener_price_min: 30.0` |
| EPSサプライズ | 5%以上 | `min_surprise_percent: 5.0` |
| ギャップ率 | 0-10% | `max_gap_percent: 10.0` |
...
| パラメータ | 値 | 設定キー |
|-----------|-----|----------|
| 損切りライン | 10% | `stop_loss: 10` |
| トレーリングMA期間 | 21日 | `trail_stop_ma: 21` |
| 部分利確ライン | 6% | `partial_profit: True` |
| 部分利確割合 | 50% | `shares // 2` |
| 最大保有日数 | 90日 | `max_holding_days: 90` |
...
### 9.1 FMP (Financial Modeling Prep) - 推奨
